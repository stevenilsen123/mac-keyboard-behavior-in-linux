...
<br><br>
This [keyd script](./default.conf) seeks to deliver all the **superior** MacOS keyboard behavior you love to the Linux desktop! It works with **both Windows and MacOS style** keyboards!

This project is tested in a [Gnome](https://www.gnome.org/) environment, on [PopOS](https://pop.system76.com/), but should work in any Linux distro.
...
1. Install [keyd](https://github.com/rvaiya/keyd) on your Linux machine
    ...
    * If you are running some other Linux distro, follow the [installation instructions](https://github.com/rvaiya/keyd#installation) on GitHub
2. Enable keyd:
...
## Usage
> ### **NOTE**
> As a base starting point, this script will swap your Ctrl and Meta (for Mac keyboards) or Ctrl and Alt (for Windows/Linux keyboards) keys in all applications. Keep this in mind if you are wondering why a particular application-specific shortcut you are used to using is not working as expected.

1. [Prerequisites](#prerequisites) first!
2. Modify the following keyboard shortcuts in your desktop environment. In Gnome, these are located in: **Settings -> Keyboard -> Keyboard Shortcuts -> View and Customize Shortcuts**
   * "Switch applications" set to "Ctrl+Tab"
   * "Switch windows of an application" set to "Ctrl+`" (that's the key above the Tab key)
   * "Hide window" set to "Ctrl+H"
3. Download this repo as a zip file to your Downloads folder

![GitHub Download](assets/github-download.png)

4. Unzip, run the init script, then clean up the mess:

```shell
unzip ~/Downloads/mac-keyboard-behavior-in-linux-main.zip -d ~/Downloads
sudo chmod +x ~/Downloads/mac-keyboard-behavior-in-linux-main/init.sh
sudo ~/Downloads/mac-keyboard-behavior-in-linux-main/init.sh &&
...
> ### **NOTE**
> The list of keyboard shortcuts below is formatted Apple-style. Use this keymap to perform these shortcuts in Linux:
> - `Command` = `Alt` key
> - `Option` = `Super` key (`Windows` key if you have a Windows keyboard)
> - `Control` = `Ctrl` key
> - `Finder` = `Nautilus` file explorer running in Gnome
...
[x] Command-Space bar: I recommend Albert. See [Prerequisites](#prerequisites) section.
[x] Control-Command-Space bar: I recommend Emote. See [Prerequisites](#prerequisites) section.
...
[ ] Control-Up: Activate Mission Control.
[ ] Option-Command-Space bar: Open a new Finder window.
[ ] Command-D: Duplicate the selected files.
[ ] Command-E: Eject the selected disk or volume.
[x] Command-F: Start a Spotlight search in the Finder window.
[x] Command-I: Show the Get Info window for a selected file.
[/] Command-R: (1) When an alias is selected in the Finder: show the original file for the selected alias. (2) In some apps, such as Calendar or Safari, refresh or reload the page. (3) In Software Update, check for software updates again. (Refresh works)
...
[x] Command-1: View the items in the Finder window as icons.
[x] Command-2: View the items in a Finder window as a list.
